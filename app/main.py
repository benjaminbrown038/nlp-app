...
raise HTTPException(status_code=400, detail="NER requires transformers. Set ENABLE_TRANSFORMERS=1 and install optional deps.")


elif req.task == "zero-shot":
if not req.candidate_labels:
raise HTTPException(status_code=400, detail="candidate_labels required for zero-shot")
if settings.enable_transformers and _has_transformers:
result = nlp_transformers.predict_zero_shot(text, req.candidate_labels)
return PredictResponse(task="zero-shot", result=result)
raise HTTPException(status_code=400, detail="Zero-shot requires transformers. Set ENABLE_TRANSFORMERS=1 and install optional deps.")
